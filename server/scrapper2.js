...
const scrapeChannel2 = async (url) => {
    ...
    const ProductData = await page.evaluate(async (data2) => {
        ...
        items.forEach((item, index) => {
            ...
            const image = item.querySelector('#maincontent > div.columns > div.column.main > div.search.results > div.products.wrapper.grid.columns4.products-grid > ol > li.item.product.product-item > div > div.product.photo.product-item-photo > a > img.product-image-photo.default_image') && item.querySelector('#maincontent > div.columns > div.column.main > div.search.results > div.products.wrapper.grid.columns4.products-grid > ol > li.item.product.product-item > div > div.product.photo.product-item-photo > a > img.product-image-photo.default_image').src;
            data2.list2.push({
                name: name,
                price: price,
                image: image
